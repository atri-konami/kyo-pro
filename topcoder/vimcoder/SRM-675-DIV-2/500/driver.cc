...
#include "ShortestPathWithMagic.cc"
...
int	k;
		...
		__in >> __expected >> dist >> k;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << dist << ", " << k << std::endl;
		std::cout.flush();

		__timer_start();

		ShortestPathWithMagic __object;
		double __actual = __object.getTime(dist, k);
