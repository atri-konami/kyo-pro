...
vector <int>	range;
		...
		__in >> __expected >> domain >> range;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << domain << ", " << range << std::endl;
		std::cout.flush();

		__timer_start();

		RelationClassifier __object;
		string __actual = __object.isBijection(domain, range);
