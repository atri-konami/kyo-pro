...
#include "LengthUnitCalculator.cc"
...
string	toUnit;
		...
		__in >> __expected >> amount >> fromUnit >> toUnit;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << amount << ", " << fromUnit << ", " << toUnit << std::endl;
		std::cout.flush();

		__timer_start();

		LengthUnitCalculator __object;
		double __actual = __object.calc(amount, fromUnit, toUnit);
